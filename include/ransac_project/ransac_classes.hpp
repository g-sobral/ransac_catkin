...
class laser
{
	private:
		...
		bool verbose;
	protected:
		laser(Publisher p, bool wp, NodeHandle node);
	public:
		static laser* uniqueInst(Publisher p, bool wp, NodeHandle node);
...
class ransacControl
{
	private:
		...
		double angularVel, dt, v_linear, lenght, KPT, KIT, KRT, KVT;
		string which_car;
	protected:
		ransacControl(Publisher p, Publisher p2, bool wp, NodeHandle node);
	public:
		ransacControl();
		static ransacControl* uniqueInst(Publisher p, Publisher p2, bool wp, NodeHandle node);
		void ransacCallback(const ransac_project::BorderLines &msg);
