...
laser::laser(Publisher p, bool wp, NodeHandle node)
{
...
laser* laser::uniqueInst(Publisher p, bool wp, NodeHandle node)
{
	if(instance == 0){
		instance = new laser(p, wp, node);
	}
...
void ransacControl::setv_linear(double x)
{
	v_linear = x;
}
...
ransacControl::ransacControl(Publisher p, Publisher p2, bool wp, NodeHandle node)
{
	pub = new Publisher();
	*pub = p;
	pub2bisec = new Publisher();
	*pub2bisec = p2;
	watchdog = new WatchDog_ransac(node);
	watchdog->StartTimer(DURATION);
...
ransacControl* ransacControl::uniqueInst(Publisher p, Publisher p2, bool wp, NodeHandle node)
{
	if(instance == 0){
		instance = new ransacControl(p, p2, wp, node);
	}
