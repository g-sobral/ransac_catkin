...
{	
	NodeHandle n;
	Publisher p2;
	ransacControl *rc2 = ransacControl::uniqueInst(p2, p2, false, n);
...
void ransacControl::ransacCallback(const ransac_project::BorderLines &msg)
{
	watchdog->IsAlive();	
	
	double rudder;
	vector_double bisectrix(4);
	vector_double biModel;
	vector<float> bisectrix2msg;
	ransac_project::Bisectrix msg2bisec;		/*mensagem que contem a linha bisetriz apenas para visualizaçao*/
	
	biModel = bisectrixLine(msg.line_left, msg.line_right);

	bisectrix[0] = -15;
	bisectrix[1] = 15;
	bisectrix[2] = -(biModel[2] + biModel[0]*bisectrix[0])/biModel[1];
	bisectrix[3] = -(biModel[2] + biModel[0]*bisectrix[1])/biModel[1];

	for(int j = 0; j < biModel.size(); j ++){
		
		bisectrix2msg.push_back(biModel[j]);
	}
	
	msg2bisec.bisectrix = bisectrix2msg;
	pub2bisec->publish(msg2bisec);
...
int main(int argc, char **argv)
{
	...
	if(which_car.compare("vero") == 0){
		...
		rc = ransacControl::uniqueInst(node.advertise<ransac_project::CarCommand>("car_command", 1), node.advertise<ransac_project::Bisectrix>("bisec_data", 1), wp, node);		
	}
		...
		rc = ransacControl::uniqueInst(node.advertise<geometry_msgs::Twist>("cmd_vel", 1), node.advertise<ransac_project::Bisectrix>("bisec_data", 1), wp, node);
	}
	...
	Subscriber sub = node.subscribe("ransac_lines", 1, &ransacControl::ransacCallback, rc);
	spin();
